...
def abrir_crud_professores(conexao):
    def listar_professores():
        ...
        try:
            ...
            cursor.execute("""
                SELECT p.id, p.siape, p.nome, p.data_nascimento, p.data_ingresso, d.nome 
                FROM professores p
                LEFT JOIN departamentos d ON p.departamento_id = d.id
            """)
            for id_, siape, nome, nascimento, ingresso, depto in cursor.fetchall():
                tree.insert("", "end", iid=id_, values=(siape, nome, nascimento, ingresso, depto or ""))
    ...
    def adicionar_professor():
        ...
        tk.Label(janela_add, text="Nome:").grid(row=1, column=0)
        ...
        tk.Label(janela_add, text="Departamento ID:").grid(row=4, column=0)
        ...
        def salvar():
            try:
                cursor = conexao.cursor()
                cursor.execute("""
                    INSERT INTO professores (siape, nome, data_nascimento, data_ingresso, departamento_id)
                    VALUES (%s, %s, %s, %s, %s)
                """, (
                    siape_entry.get(),
                    nome_entry.get(),
    ...
    def excluir_professor():
        ...
        if confirmar:
            try:
                cursor = conexao.cursor()
                cursor.execute("DELETE FROM professores WHERE id = %s", (item,))
                conexao.commit()
                listar_professores()
            except Exception as e:
