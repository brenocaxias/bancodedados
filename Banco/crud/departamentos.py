...
def abrir_crud_departamentos(conexao):
    def listar_departamentos():
        ...
        try:
            ...
            cursor.execute("SELECT id, codigo, nome FROM departamentos")
            for id_, codigo, nome in cursor.fetchall():
                tree.insert("", "end", iid=id_, values=(codigo, nome))
    ...
    def adicionar_departamento():
        ...
        if codigo and nome:
            try:
                cursor = conexao.cursor()
                cursor.execute("INSERT INTO departamentos (codigo, nome) VALUES (%s, %s)", (codigo, nome))
                conexao.commit()
    ...
    def excluir_departamento():
        ...
        if confirmar:
            try:
                cursor = conexao.cursor()
                cursor.execute("DELETE FROM departamentos WHERE id = %s", (item,))
                conexao.commit()
                listar_departamentos()
            except Exception as e:
